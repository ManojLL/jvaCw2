...
public class Train extends Application {
    ...
    public void start(Stage primaryStage) {
        ...
        //[6] user details 6
        String[][][][] booking = new String[2][30][SEAT_CAPACITI][6];
    ...
    private void addSeat(Stage stage, Scene scene, int diff, LocalDate date, int x, String[][][][] book, String desta) {
        ...
        pane.getChildren().addAll(idText, idNum, slSeat, name, address, contact, nameText, addressText, contactText, email, emailText, backB, closeStage, label, booking, choiceBox);
        label.setLayoutX(60);
        ...
        idText.setLayoutY(70);
        addressText.setLayoutX(130);
        addressText.setLayoutY(100);
        contactText.setLayoutX(130);
        contactText.setLayoutY(130);
        emailText.setLayoutX(130);
        emailText.setLayoutY(160);
        choiceBox.setLayoutX(170);
        choiceBox.setLayoutY(210);
    ...
    private void addToArray(String seatNum, String name, String id, String contact, String email, String address, String[][][][] book, int x, int diff, Stage stage, Scene scene) {
        ...
        book[x - 1][diff - 1][Integer.parseInt(seatNum) - 1][3] = address;
        book[x - 1][diff - 1][Integer.parseInt(seatNum) - 1][4] = contact;
        book[x - 1][diff - 1][Integer.parseInt(seatNum) - 1][5] = email;
        stage.setScene(scene);
    }
        ...
        if (run[0]) {
            ...
            while (true) {
                ...
                for (int x = 0; x < SEAT_CAPACITI; x++) {
                    if (name.equalsIgnoreCase(booking[choseDeasta[0] - 1][difference[0] - 1][x][1])) {
                        System.out.println(booking[choseDeasta[0] - 1][difference[0] - 1][x][1] + " - " + booking[choseDeasta[0] - 1][difference[0] - 1][x][0] + " - " +
                                booking[choseDeasta[0] - 1][difference[0] - 1][x][2] + " - " + booking[choseDeasta[0] - 1][difference[0] - 1][x][3] + " - " + booking[choseDeasta[0] - 1][difference[0] - 1][x][4] +
                                " - " + booking[choseDeasta[0] - 1][difference[0] - 1][x][5]);
                        find = true;
                    }
                }
                if (!find) {
        ...
        if (run[0]) {
            ...
            for (int i = 0; i < SEAT_CAPACITI; i++) {
                if (booking[choseDeasta[0] - 1][difference[0] - 1][i][1] != null) {
                    names.add(booking[choseDeasta[0] - 1][difference[0] - 1][i][1]);
